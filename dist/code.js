...
(() => {
  ...
  var LOCAL_PRESETS = [
    {
      ...
      "palettes": {
        ...
        "white-alpha": {
          ...
          "A100": "rgba(255, 255, 255, 0.2)",
          "A200": "rgba(255, 255, 255, 0.5)",
          "A400": "rgba(255, 255, 255, 0.7)",
          "A700": "rgba(255, 255, 255, 0.95)"
        }
      },
      "neutralOptions": [
        "neutral",
    ...
    },
    {
      "id": "polaris",
      "label": "Polaris",
      "description": "Local preset loaded from Polaris - colors.json.",
      "palettes": {
        "1": {
          "$type": "color",
          "$value": "#fbfdff"
  ...
  var BUILTIN_PRESETS = LOCAL_PRESETS;
  ...
  function expandSteps(baseSteps, count) {
    const min = baseSteps[0];
    const max = baseSteps[baseSteps.length - 1];
    const generated = [];
    for (let i = 0; i < count; i += 1) {
      generated.push(Math.round(min + (max - min) * i / Math.max(1, count - 1)));
    }
    const unique = Array.from(new Set(generated)).sort((a, b) => a - b);
    while (unique.length < count) {
      unique.push(unique[unique.length - 1] + 1);
    }
    if (baseSteps.includes(500) && !unique.includes(500) && count >= 3) {
      let replaceIndex = 1;
      let best = Number.MAX_SAFE_INTEGER;
      for (let i = 1; i < unique.length - 1; i += 1) {
        const delta = Math.abs(unique[i] - 500);
        if (delta < best) {
          best = delta;
          replaceIndex = i;
        }
      }
      unique[replaceIndex] = 500;
    }
    return unique.sort((a, b) => a - b).slice(0, count);
  }
  function deriveShadeSteps(pattern, shadeCount) {
    const base = basePatternSteps(pattern);
    const requested = clampNumber(shadeCount, 3, 16, 11);
    if (requested <= base.length) return pickSubset(base, requested);
    return expandSteps(base, requested);
  }
  ...
  function applyDarkModeTransformForReference(tokenName, pluginReference, neutralPalette, preset) {
  ...
  function convertKigenReferenceToPluginRef(tokenName, kigenReference, uiMode, preset, neutralPalette, shadeSteps, useBrandForAccent) {
    ...
    if (segments[0] === "b&w") {
      ...
      return uiMode === "dark" ? applyDarkModeTransformForReference(tokenName, resolved2, neutralPalette, preset) : resolved2;
    }
  ...
  function buildGenerationOptions(rawPayload) {
    ...
    const shadeCount = clampNumber(input.shadeCount, 3, 16, 11);
  ...
  function ensureSingleDefaultMode(collection, report) {
    ...
    if (!modeMap[DEFAULT_MODE]) {
      if (collection.modes.length === 1) {
        ...
        report.migrations.push(`Mode renomm\xC3\xA9 (${collection.name}): ${previous} -> ${DEFAULT_MODE}`);
      } else {
        collection.addMode(DEFAULT_MODE);
        report.migrations.push(`Mode ajout\xC3\xA9 (${collection.name}): ${DEFAULT_MODE}`);
      }
    }
    for (const mode of [...collection.modes]) {
      if (mode.name !== DEFAULT_MODE && collection.modes.length > 1) {
        collection.removeMode(mode.modeId);
        report.migrations.push(`Mode supprim\xC3\xA9 (${collection.name}): ${mode.name}`);
      }
    }
    ...
    if (!modeId) {
      throw new Error(`Impossible de d\xC3\xA9finir le mode ${DEFAULT_MODE} pour ${collection.name}`);
    }
    return modeId;
  }
  ...
  async function applyTypographyTextStyles(options, report, typographyCollection, variableIndex) {
    var _a;
  ...
  async function applyGeneration(options) {
    var _a, _b, _c;
    ...
    if (colorModesResult == null ? void 0 : colorModesResult.missing.length) {
    ...
    if (options.createTextStyles) {
      await applyTypographyTextStyles(options, report, (_c = collectionsByName.get(COLLECTIONS.typography)) != null ? _c : null, variableIndex);
    } else {
  ...
  async function postFontFamiliesList() {
    try {
      ...
      const families = Array.from(new Set(fonts.map((font) => font.fontName.family).filter(Boolean))).sort(
