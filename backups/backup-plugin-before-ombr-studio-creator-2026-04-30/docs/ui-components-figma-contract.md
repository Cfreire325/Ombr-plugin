...
- `Button` (Figma node `330:3164`)
  - Code: `.ds-button`
  - Variants: `primary`, `secondary`, `ghost`, `destructive` (Figma `Error`)
  - Sizes: `large`, `regular`, `small`, `mini`
  - Etats: `default`, `hover`, `active`, `focus-visible`, `disabled`
  - Options: `.ds-button__icon`, `.ds-button--full-width`
  - Typographie: `letter-spacing: 0.07px` pour `large`, `regular`, `small`; `letter-spacing: 0.18px` pour `mini`

- `IconButton` (Figma node `330:1095`)
  - Code: `.ds-icon-button`
  - Variants: `primary`, `secondary`, `ghost`, `destructive`
  - Sizes: `large`, `regular`, `small`, `mini`
  - Etats: `default`, `hover`, `active`, `focus-visible`, `disabled`
  - Accessibilite: `aria-label` obligatoire sur les boutons sans texte visible
...
## Regles QA Button / IconButton

- `fullWidth` est optionnel: ne jamais forcer globalement `width: 100%` sur `.ds-button`; utiliser une classe explicite comme `.ds-button--full-width`.
- L'etat `active` / `pressed` ne doit pas modifier la geometrie du bouton: pas de deplacement, resize, padding, border ou layout shift.
- Tout bouton icon-only doit avoir un `aria-label` explicite, meme si l'icone semble evidente visuellement.
- Eviter les styles locaux de boutons quand `.ds-button` ou `.ds-icon-button` couvre le besoin.
- Centraliser les styles `disabled` dans le DS autant que possible; les ecrans ne doivent ajouter que des contraintes metier comme l'activation/desactivation.
