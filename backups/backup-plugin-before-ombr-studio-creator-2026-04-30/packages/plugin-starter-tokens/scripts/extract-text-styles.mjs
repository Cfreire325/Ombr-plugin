...
function main() {
  ...
  const inputCandidates = [
    resolve(root, "Styles de texte.json"),
    resolve(root, "assets/figma/exports/Styles de texte.json"),
  ];
  ...
  if (!inputPath) {
    if (existsSync(outPath)) {
      console.info(
        `[extract-text-styles] Optional source not found; keeping existing ${outPath}. Expected one of: ${inputCandidates.join(" | ")}`,
      );
      return;
    }
