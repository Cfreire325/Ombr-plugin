...
const PRESET_DIR_CANDIDATES = [
  ...
  "Colors -  Preset Palette",
  "assets/figma/frames/Colors - Preset Palette",
  "assets/figma/frames/Colors -  Preset Palette",
];
const EXCLUDED_PRESET_IDS = new Set(["polaris"]);
const GUARANTEED_PRESET_IDS = ["material", "untitled-ui"];
...
function readExistingGeneratedPresets(filePath) {
  if (!existsSync(filePath)) return [];
  try {
    const text = readFileSync(filePath, "utf8");
    const marker = "export const LOCAL_PRESETS: PresetDefinition[] = ";
    const start = text.indexOf(marker);
    if (start < 0) return [];
    const after = text.slice(start + marker.length);
    const end = after.lastIndexOf(";");
    if (end < 0) return [];
    const jsonPayload = after.slice(0, end).trim();
...
function buildPresetFromFile(filePath, fileName) {
  ...
  for (const palette of palettesRaw) {
    ...
    const isMaterialAlphaPalette = idBase === "material" && /(^|-)alpha($|-)/.test(baseKey);
    if (isMaterialAlphaPalette) {
      continue;
    }
...
function generate() {
  ...
  const existingPresets = readExistingGeneratedPresets(outPath);
  ...
  for (const id of GUARANTEED_PRESET_IDS) {
    if (presets.some((preset) => preset.id === id)) continue;
    if (existingById.has(id)) {
      presets.push(existingById.get(id));
    }
  }
